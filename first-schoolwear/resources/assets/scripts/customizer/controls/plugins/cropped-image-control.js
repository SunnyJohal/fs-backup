...
import TTCroppedImageControl from '../containers/TTCroppedImageControl/TTCroppedImageControl';
 ...
 */
api.TTCroppedImageControl = api.CroppedImageControl.extend({
  /**
   * Initialize.
   *
  ...
  initialize(id, settingConfig) {
    ...
    wp.customize.CroppedImageControl.prototype.initialize.call(
      control,
      id,
  ...
  getSettingParamsFromConfig(id, config) {
    ...
    const controlProps = {
      ...
      button_labels,
      width: input_attrs.width,
      height: input_attrs.height,
      canSkipCrop: input_attrs.can_skip_crop
    };
  ...
  /**
   * Set Image From Attachment
   *
   * @description Updates the setting and re-renders the
   *   control UI. Overrides the parent method to store
   *   the attachment url insead of the attachment id.
   *
  ...
  destroy() {
    ...
    if (wp.customize.CroppedImageControl.prototype.destroy) {
      wp.customize.CroppedImageControl.prototype.destroy.call(control);
    }
  }
});

wp.customize.controlConstructor.tt_cropped_image = api.TTCroppedImageControl;
